...
db = databank.retrieveColumns(db,1);
...
db = databank.merge("horzcat", db, dbc, ...
  "missingField", NaN);

% Rescale debt variables
db.debt_y     = db.debt_y / 4;
db.debt_fcy_y = db.debt_fcy_y / 4;
db.debt_lcy_y = db.debt_lcy_y / 4;
...
rprt = codes.reporting.addTablePage(optsCurr, rprt, m, db, varNames, tableTitle, legends);
...
rprt = codes.reporting.addPage(optsCurr, rprt, m, db, varNames, figureTitle, legends);
...
rprt = codes.reporting.addTrendsAndGaps(optsCurr, rprt, m, db, db, optsFR.plotRange, legends, ...
  "forecast");
...
rprt = codes.reporting.addShocks(optsCurr, rprt, m, db, optsFR.plotRange, legends, "forecast");
...
% fileName = fullfile(opts.mainDir, "reports", "forecastCompareReport.pdf");
