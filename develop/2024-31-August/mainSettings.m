...
opts.roundId        = "2024 Sept main round";
opts.modelFile      = "minecofin";
opts.mainDataFile   = "2024-08-31"; % name of data/Rwametric file 
opts.addDataFile    = "2024-03-03"; % unchanged GPM file (real interest), old file name
...
% Filtered range
opts.filterHistory.range = qq(2006,1) : qq(2024,1); % default 2024Q1 if data; extend filter for shocktuning +2Q
...
% Table range
opts.filterHistory.rangeTable = qq(2019, 3) : qq(2024, 1); % default 2024Q1; extend filter for shocktuning, +2Q

% Plot range
opts.filterHistory.rangePlot = qq(2015, 1) : qq(2024, 1); % same
...
opts.histForecast.range = qq(2016, 1) : qq(2024, 1);
...
% Horizon of forecast
opts.forecast.range = qq(2024, 2) : qq(2028, 4); % extended forward each Q if new data
...
opts.forecastReport.highlightRange = qq(2017,1) : qq(2024,1); %  

% Range of the tables in the forecast report
opts.forecastReport.tableRange = qq(2023,4) : qq(2027,4); % extend 2026Q4

% Range for annualized series
opts.forecastReport.tableRange_ann = yy(2022) : yy(2028);
