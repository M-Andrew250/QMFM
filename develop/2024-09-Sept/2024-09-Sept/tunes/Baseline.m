...
m.ss_grev_y_str = 23;   % kept same as historical parameter s-state
m.ss_oexp_y_str = 6-1;  % 1 pps down from historical s-state
m.ss_gdem_y_str = 26-1; % 2 pps down from historical s-state
...
% tunes.shock_dl_cpi_core = Series(qq(2024,4):qq(2026,2), [0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4]); 

% 3b. CPI_food soft-tune 2024Q4 a.f. if further shock expected  
%rngExog = qq(2024, 4): qq(2024,4);
% tunes.shock_dl_cpi_food = Series(qq(2024,4):qq(2026,1), [1, 1, 1, 0.5, 0.5, 0.5]); 
...
rngExog = qq(2024,3) : qq(2024,4);
tunes.shock_l_inv_gap(rngExog) = [ 0.10,  -0.95]; % need to be read from ext.filter results
tunes.shock_l_exp_gap(rngExog) = [ 0.21,  -2.1]; % 
...
pln = exogenize(pln,  rngExog, 'def_y');
pln = endogenize(pln, rngExog, 'shock_gdem_y_discr');
tunes.def_y(rngExog) = [8.7, 8.7, 9.1, 9.1, 7.8, 7.7, 7.5, 7.4]; % ann.8.9% & 7.6%; bit higher (lower) in 2d half FY2024-25 (2025/26)
...
rngExog = qq(2024, 3): qq(2027, 2);
pln = exogenize(pln,  rngExog, 'grev_y');
pln = endogenize(pln, rngExog, 'shock_grev_y_discr');
tunes.grev_y(rngExog) = [18.1, 18.3, 18.5, 18.7, 19.1, 19.4, 19.7, 19.8, 19.9, 20.0, 20.2, 20.3]; % FYannual 18.4, 19.5, 20.1%
