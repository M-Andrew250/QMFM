...
for i = 1 : scenNum
  ...
  dbAuxFcast  = reporting(mi, dbFcasti, optsRF.plotRange);
  dbFcasti    = dboverlay(dbFcasti, dbAuxFcast);
  
  % ----- Save results to csv file -----
  
  codes.writeMessage(...
    "calcForecast: scenario " + scenarios(i) + ": writing results to CSV ..." ...
    );
  ...
  varNames = string(fieldnames(dbDecompFilt));
  for v = varNames(:)'
    dbDecomp.(v).decompSeries{i}    = [dbDecompi.(v).decompSeries{1}; dbDecompFilt.(v).decompSeries{1}];
    dbDecomp.(v).decompContribs{i}  = [dbDecompi.(v).decompContribs{1}; dbDecompFilt.(v).decompContribs{1}];
  end
