...
p.ss_grants   =  5; % ss (in % GDP) grants from validated expected donor support
...
p.v1 = 0.042; % response cyclical rev to consgap times share rev in GDP
p.v2 = 0.021; % response cyclical rev to importgap times share rev in GDP
p.v3 = 0.07; % response cyclical rev to outputgap times share rev in GDP, lower:given v1 and v2
p.v4 = 0.99; % lag coefficient structural rev

p.u1 = 0;0.9; % lag coefficient cyclical other exp
p.u2 = 0.036;0.06; 0.0; % response cyclical other exp to outputgap times share other exp in GDP (<0 in eq)
p.u3 = 0.95;  % lag coefficient structural other exp

p.t1 = 0;0.9;  % lag coefficient cyclical govt dem G&S
p.t2 = 0.115; 0.23; 0.0; % response cyclical govt dem G&S to outputgap times share govt dem in GDP (<0 in eq)
...
p.a5_inv   = 0.15; 0.3; % Fiscal impulse; coeff=impact/cons share GDP=0.04/0.13=0.3;
% divide by 2 as fiscal impulse eq. has factor 2: ie sum discr + ch struct def
...
p.gamma_k = 0.8; % was 0.5, we need higher persistence
p.k_bar   = 100*log(6500); % was 95 (mln$); current stock est. at $6.5bln
