...
% 1b. CPI_food hard-tune with 2025Q1 data
rngExog = qq(2025, 1);
...
%1c. CPI_ener hard-tune with 2025Q1 data 
rngExog = qq(2025, 1);
...
pln = exogenize(pln,  rngExog, 'i');
pln = endogenize(pln, rngExog, 'shock_i');
tunes.i(rngExog) = 100 * log(1 + 6.8/100);
...
% soft-tune private investment shock, to reflect Bugesera esp from govt netLending (oexp) +6% GDP in 2025Q3-4,
% but effect spread over 2025Q3-4, 2026Q1-2,2027Q1-2 (so 1.5%), markup by 1(investment/GDP)=1/0.2=5
rngExog = qq(2025,3) : qq(2027,2);
tunes.shock_l_inv_gap(rngExog) = [2, 5, 15, 8, 8, 8, 5, 2];
...
tunes.def_y(rngExog) = [7.8, 7.8, 14.0, 14.0, 6.0, 6.0, 10.0, 10.0, 6.0, 6.0, 6.1, 6.1];
...
tunes.oexp_y(rngExog) = [12.0, 12.0, 4.0, 4.0, 8.0, 8.0, 6.0, 6.0];
...
pln = endogenize(pln, rngExog, 'shock_grev_y_discr');
tunes.grev_y(rngExog) = [19.0, 18.6, 18.6, 18.7, 18.9, 19.0, 19.3, 19.6, 19.8, 20.1, 20.2, 20.2];
