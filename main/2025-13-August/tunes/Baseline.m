...
% Policy expected interest rate (MPC decision is effective next 3Mo)
rngExog = qq(2025, 3);
pln = exogenize(pln,  rngExog, 'i');
pln = endogenize(pln, rngExog, 'shock_i');
...
% pln = exogenize(pln,  rngExog, 'd4l_y');
% pln = endogenize(pln, rngExog, 'shock_l_cons_gap');
% tunes.d4l_y(rngExog) = 100 * log(1 + [8.7, 7.1]/100); % Tune with nowcast 
...
rngExog = qq(2025, 2) : qq(2025, 3);
tunes.shock_l_inv_gap(rngExog) = [ 0.72016,    -0.16649]; % need to be read from ext.filter results
tunes.shock_l_exp_gap(rngExog) = [ 1.5924,     -0.36919]; % 
tunes.shock_l_imp_gap(rngExog) = [ -0.055453,  -0.0014989]; %
...
tunes.def_y(rngExog) = [7.8, 7.8, 10.0, 9.8, 10.2, 10.0, 9.3, 9.0, 7.2, 6.9, 6.4, 6.3];
...
pln = endogenize(pln, rngExog, 'shock_grev_y_discr');
tunes.grev_y(rngExog) = [18.5, 19.0, 18.5, 18.7, 18.9, 19.1, 19.3, 19.6, 19.8, 20.1, 20.2, 20.3];
%
