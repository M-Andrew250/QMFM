...
% codes.reportModel() creates a report of the main model properties for all parametrizations.
%
% Usage: codes.reportModel()
%
% codes.reportModel loads the model, the observed and filtered data, and
% creates a PDF report depicting selected model properties and evaluation
...
% - opts.steady.variables:      list of variables in the steady state table
% - opts.irf.horizon:           horizon of the impulse response functions
% - opts.irf.shocks:            shock in the IRF charts
% - opts.irf.variables:         variables in the IRF charts
% - opts.irf.shockSize:         size if the shock in the IRF charts (can be @auto,
%                               to use calibrated std-s)
% - opts.irf.subplot:           subplot grid of the IRF charts (can be determined
%                               automatically)
% - opts.varDecomp.horizon:     horizon in VD charts
% - opts.varDecomp.variables:   variables in VD charts
% - opts.varDecomp.contribs:    number of shocks in the VD charts (the goes to the
%                               "rest")
% - opts.varDecomp.histRange:   the range on which the historical std-s are
%                               calculated
% - opts.stdComp.variables:     variables in the variable std comparison table
% - opts.stdComp.shocks:        shock in the shock std comparison table
% - opts.stdComp.histRange:     the range on which the historical std-s are
%                               calculated
%
% The PDF report is saved as opts.resultsDirPdf/modelReport.pdf
% 
% See also: codes.readModel, codes.filterHistory, codes.reportFilter
