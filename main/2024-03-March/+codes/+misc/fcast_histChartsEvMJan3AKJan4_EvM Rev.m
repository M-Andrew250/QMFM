...
highlight(HighRange);
title(ygap_decomp.figureName, 'FontSize',12,'interpreter', 'none');
...
plot(plotRange, db.dl_s_tar, 'linewidth', 2); %'color', 'r'),...
  legend ('ER actual', 'ER target ', 'interpreter', 'none','Fontsize',12,...
      'location', 'southoutside', 'orientation', 'horizontal', 'fontsize', 12);
zeroline;
ylim ([-1,16]);
grid on
...
grid on
highlight(HighRange); %,'EdgeColor','m','LineWidth',1.5);
legend ('RER, ann. QoQ %','RER trend, ann. QoQ %', 'RER gap %', 'interpreter', 'none',...
    'location', 'southoutside', 'orientation', 'horizontal', 'fontsize', 12);
title('RER depreciation rate vs. trend and RER gap, %', 'interpreter', 'none', 'FontSize',12);
fileName = "RER_QQ %";
codes.utils.saveEmf(opts, gcf, fileName);
...
legend ('Nominal ER', 'Forein CPI','CPI_core', 'interpreter','none',...
  'location', 'southoutside', 'orientation', 'horizontal', 'fontsize', 12);
highlight(HighRange)
title('Real Exchange Rate (RER), Q-on-Q % (dl_z)', 'fontsize', 12,...
  'interpreter','none');
fileName = "RER_decomp";
codes.utils.saveEmf(opts, gcf, fileName);
...
figure("visible", "off");
ls= dbDecomp.dl_s_tar;
barcon(plotRange,ls.decompContribs{1}, ...
  'colormap', parula);
hold on
plot(plotRange,ls.decompSeries{1}, ...
  'linewidth', 3, 'color',    'w');
plot(plotRange,ls.decompSeries{1}, ...
  'linewidth', 1, 'color', 'k');
grid on
legend(ls.legendEntries, ...
  'location', 'southoutside', 'orientation', 'horizontal', 'FontSize',12);
highlight(HighRange);
title(ls.figureName, 'FontSize',12,'interpreter', 'none');
fileName = "ER_decomp";
codes.utils.saveEmf(opts, gcf, fileName);
...
figure("visible", "on");
plot(plotRange, db.i, 'linewidth', 2); % 'color', 'k');
hold on
...
plot(plotRange, [(db.r4_gap + db.r_tnd + db.ss_prem_d), db.r_tnd + db.prem_d,...
  db.r_tnd], 'linewidth', 2);
hold on
plot(plotRange,db.r,'linewidth', 2, 'color', 'b');

legend ('Real lending rate', 'Real lending rate trend', 'RIR trend','RIR (IB)',...
...
grid on
highlight(HighRange) %,'EdgeColor','m','LineWidth',1.5);
title('Fiscal stance: effect of govt & private demand on output, % GDP',...
    'FontSize',12 ,'interpreter', 'none');
fileName = "outputgap_gdem_fiscimp_effects.emf";
codes.utils.saveEmf(opts, gcf, fileName);
%========================================================================================================================================================================================================
%% Fiscal impulse
% 1.Indirect effect of Govt demand on final demand(~private demand) 
...
codes.utils.saveEmf(opts, gcf, fileName);
%===================================================================================================================================
%% 2. Direct effect of Govt demand on final demand(~Govt demand of G&S)

figure("visible", "on");
govDem= dbDecomp.l_gdem;
barcon(plotRange, govDem.decompContribs{1}, ...
  'colormap', parula);
hold on
plot(plotRange, govDem.decompSeries{1}, ...
  'linewidth', 3, 'color',    'w');
plot(plotRange, govDem.decompSeries{1}, ...
  'linewidth', 1, 'color',    'k');
grid on
legend(govDem.legendEntries, ...
  'location', 'southoutside','orientation', 'horizontal');
highlight(HighRange);
title(govDem.figureName, 'FontSize', 12,...
  'interpreter', 'none');
...
codes.utils.saveEmf(opts, gcf, fileName);
%===============================================================================================================================
%% Budget decifit decompositions (def_y & def_y_scd)

figure("visible", "on");
def_decomp = dbDecomp.def_y_cyc;
...
%highlight(HighRange)
legend 'RER depr. Q-on-Q %' 'ER depr. Q-on-Q %' 'CPI_core, Q-on-Q %'
...
legend ('Level, %', 'Trend', 'interpreter', 'none','Fontsize',10,...
    'location', 'southoutside', 'orientation', 'horizontal');
title('Trading partner demand vs. trend, Y-on-Y %','interpreter', 'none','FontSize',12);
...
legend ('Level %', 'Trend', 'interpreter', 'none','Fontsize',10,...
    'location', 'southoutside', 'orientation', 'horizontal');
title('Foreign food price (WEO) vs. trend, Y-on-Y %','interpreter', 'none','FontSize',12);
...
legend ('Level, %', 'Trend', 'interpreter', 'none','Fontsize',10,...
    'location', 'southoutside', 'orientation', 'horizontal');
title('Foreign energy price (WEO), Y-on-Y %','interpreter','none','FontSize',12);
...
grid on
highlight(HighRange); %,'EdgeColor','m','LineWidth',1.5);
legend ('US RIR, %', 'interpreter', 'none','Fontsize',10,...
    'location', 'southoutside', 'orientation', 'horizontal');
title('Foreign Real Interest Rate trend (RIR), %','FontSize',12);
...
zeroline;
grid on
highlight(HighRange); %,'EdgeColor','m','LineWidth',1.5);codes.fi
legend ('Foreign CPI, %', 'interpreter', 'none','Fontsize',10,...
    'location', 'southoutside', 'orientation', 'horizontal');
title('Trading partner CPI, Y-on-Y %','interpreter', 'none','FontSize',12);
fileName = "rstar_and_trend.emf";
codes.utils.saveEmf(opts, gcf, fileName);
