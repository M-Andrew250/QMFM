...
opts.roundId        = "2024 March main round";
opts.modelFile      = "minecofin";
opts.mainDataFile   = "2024-03-13"; % name of data/Rwametric file 
opts.addDataFile    = "2024-03-03"; % unchanged GTM file (real interest), old file name
...
opts.parameterNames   = "setparam"; % should be "setparam", not setparam_fisc_Rev
...
opts.reportModel.varDecomp.histRange = qq(2006,1) : qq(2023,3);
...
% Range of comparison
opts.reportModel.stdComp.range = qq(2006,1) : qq(2023,3);

%% Historical filter options

% Filtered range
opts.filterHistory.range = qq(2006,1) : qq(2023,3); % default 2023Q3 if data; extend filter for shocktuning +2Q
...
% Table range
opts.filterHistory.rangeTable = qq(2019,1) : qq(2023,3); % default 2023Q3; extend filter for shocktuning, +2Q

% Plot range
opts.filterHistory.rangePlot = qq(2015,1) : qq(2023,3); % same
...
% Horizon of forecast
opts.forecast.range = qq(2023,4) : qq(2028,4); % extended forward each Q if new data
...
opts.forecastReport.highlightRange = qq(2015,1) : qq(2023,3); %  

% Range of the tables in the forecast report
opts.forecastReport.tableRange = qq(2022,4) : qq(2026,4); % extend 2026Q4
