...
% codes.utils.processOptions processes the options set in mainSettings().
% 
% Usage: opts = codes.utils.processOptions(opts)
%
% Processing involves determining the main QMFM directory, setting absolute
% paths to folders and files, creating possibly non-exitent results folders, creating
% function handles to tune functions, and getting the options of the
% comparison round. Also, report styles are set here.

% ------------- Directories and files -------------

mainDirFolders = split(opts.mainDir, filesep);

opts.QMFMDir = join(mainDirFolders(1:end-2), filesep);

opts.modelDir = fullfile(opts.mainDir, "model");

opts.modelFile      = fullfile(opts.modelDir, opts.modelFile + ".model");
...
if opts.resultsOutside
  resultsDir = fullfile(opts.QMFMDir, "results", join(mainDirFolders(end-1:end), filesep));
else
...
% ------------- Create the parameter function handles -------------

cd(opts.modelDir)
for pn = opts.parameterNames(:)'
  opts.parameters.(pn) = str2func("@" + pn);
end
cd(opts.mainDir)

% ------------- Create the tune function handles -------------

cd(opts.tunesDir)

opts.tunes.Historical = str2func("@Historical");
for sn = opts.forecast.scenarioNames(:)'
  opts.tunes.(sn) = str2func("@" + sn);
end

cd(opts.mainDir)

% ------------- Use parameter/scenario names as legends if parameterLegends/scenarioLegends are empty -------------

if isempty(opts.parameterLegends)
  opts.parameterLegends = opts.parameterNames;
  if all(startsWith(opts.parameterLegends), "setparam")
    opts.parameterLegends = extractAfter(opts.parameterLegends, "setparam");
  end
end

if isempty(opts.forecast.scenarioLegends)
  opts.forecast.scenarioLegends = opts.forecast.scenarioNames;
end

% ------------- Options of the comparison round -------------

% Determine if procesOptions has been called from the current mainSettings, or from the mainSettings
% of a future round which compares to the current round. Do not execute the mainSettings of the
% comparison round in the latter case, to avoid recursively executing all previous
% mainSettings functions.

% Names of calling functions (with full path)
stack = dbstack("-completenames");

% Check if the top level caller function is in the current main directory
callerCurrent = contains(stack(end).file, opts.mainDir);
