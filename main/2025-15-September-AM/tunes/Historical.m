...
dbObs.tune_d4l_y (rngTune) = 100 * log(1 + [8.8,7.7]/100); % Nowcast March'25 %%%%7.3, 
% 
...
% 
rngTune = qq(2025, 2): qq(2025, 3);
%dbObs.tune_dl_s(rngTune) = 100 * log(1 + 0.052);
dbObs.tune_dl_s(rngTune) = [725.78, 727.51];
%tunes.l_s(rngTune) = [725.78, 727.51];

% rngTune = qq(2025, 4);
% dbObs.tune_dl_s(rngTune) = 100 * log(1 + 0.052);
% 
% %% interest IB rate hard-tune 2025Q4 for extended filter as also tuned in BL forecast
% %% (no change CBR, so IB 0.3% above av. CBR=6.5% 2025Q1)
%
rngTune = qq(2025, 2) : qq(2025, 3);
dbObs.obs_i(rngTune) = [6.5, 5.4];
%
% rngTune = qq(2025, 4);
% dbObs.obs_i(rngTune) = 100 * log(1 + 6.7/100);
% 
%
...
rngTune = qq(2025, 2) : qq(2025, 3);
dbObs.tune_grev_y(rngTune) = [19.0,18.6]; %19.0,
% % 
