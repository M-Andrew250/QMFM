...
if optsRH.obs.run
  ...
  codes.writeMessage(mfilename + ": adding observed data to the history report ...");
...
if optsRH.trends.run
  ...
  codes.writeMessage(mfilename + ": adding trends and gaps to the history report ...");
  % if plot should start later, change optsFH.range into optsFH.rangePlot
...
if optsRH.eqDecomps.run
  ...
  codes.writeMessage(mfilename + ": adding equation decompositions to the history report ...");
...
if optsRH.shockDecomps.run
  ...
  codes.writeMessage(mfilename + ": adding shock decompositions to the history report ...");
...
if optsRH.shocks.run
  ...
  codes.writeMessage(mfilename + ": adding shocks to the history report ...");
...
codes.writeMessage(mfilename + ": compiling the history report ...");
